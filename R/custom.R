...
#'
#' @description
#' For any requirement not currently in place in the \code{vstsr} package, then this function will allow you to use the relevant
#' API call without any extra requirements.
#'
...
#' @param body check \code{\link[httr]{VERB}} for more details. If the object is a named list, then it will be transformed into a JSON
#' string so that can be added to the call.
#' @param query a list of extra parameters that can be sent to the API call. If not required then leave as \code{NULL}
#'
#' @export
vsts_run_command <- function(url, verb, auth_key, body = FALSE, query = NULL) {
